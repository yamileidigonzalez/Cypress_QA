...
const { description } = require("platform");
describe('Protocolos', () => {
    const tiempo = 1000;
    ...
    it('Debería añadir un nuevo [Elemento]', () => {
      ...
      cy.fixture('9_Protocolos.json').then((protocolo) => {
        protocolo.forEach((p) => {
          let id = p.id;
          let descripcion = p.descripcion;  // Sin tildes
          let version = p.version;          // Sin tildes
          //Boton añadir
    ...
    it('Debería modificar un [Elemento]', () => {
      ...
      cy.Editar_Protocolos("1", "21", "20")
      cy.Volver('.w-full > .p-ripple', tiempo)      
    });
